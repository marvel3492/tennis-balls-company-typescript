...
import { isPromotion, isPromotionArray } from '../../../shared/data/PromotionData.js';
var router = Router();
...
router.get('/', adminOnly, function(_req, res, _next) {
    ...
    renderAllRecords(res, query, isPromotionArray);
});
...
router.get('/:recordid/show', function(req, res, _next) {
    ...
    renderOneRecord(req, res, query, isPromotion);
});
...
router.post('/', adminOnly, function(req, res, _next) {
    let imageId = req.body.image_id ? req.body.image_id : null;
