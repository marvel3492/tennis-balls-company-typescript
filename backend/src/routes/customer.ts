...
import type { UserData } from '../../../shared/data/UserData.js';
import type { LoginData } from '../../../shared/data/LoginData.js';
var router = Router();
...
router.get('/credentials', function(req, res, _next) {
    let userData: UserData = {customer_id: req.session.customer_id ?? 0, custname: req.session.custname ?? "", isadmin: req.session.isadmin ?? 0};
    res.json(userData);
});
...
router.post('/login', guestOnly, function(req, res, _next) {
    ...
    db.all(query, [req.body.username], (err, result) => {
        if (err) {
            ...
            if (!result[0]) {
                let loginData: LoginData = {success: false, message: "Wrong Username"};
                ...
                compare(req.body.password, result[0].password, function(err, result2) {
                    if (err) {
                        ...
                        let loginData: LoginData = {success: false, message: "Wrong Password"};
                        res.status(401).json(loginData);
                        ...
                        let loginData: LoginData = {success: true, message: "Logging in"};
                        res.json(loginData);
                    }
                });
            }
...
function generatePassword(req: Request, res: Response, callback: (hash: string) => void) {
    genSalt(10, (err, salt) => {
        if (err) {
            renderError(res, err);
